...
import { faToggleOn, faToggleOff, faPenToSquare, faTrashCan } from '@fortawesome/free-solid-svg-icons';
...
const SingleItem = ({ _id, name, descricao, preco, fotoURL, ativo: initialAtivo }) => {
  ...
  const toggleAtivo = async () => {
    try {
      const response = await axios.put(`http://localhost:8000/api/cardapio/item/${_id}`, {
        ativo: !ativo,
      });
